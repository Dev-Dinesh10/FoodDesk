...
const EditItemModal: React.FC<EditItemModalProps> = ({ visible, onClose, item, onSave }) => {
  ...
  useEffect(() => {
    if (item) {
      setForm({
        ...
        foodType: item.foodType || (item.isVegetarian ? 'veg' : 'nonveg'),
        basePrice: item.basePrice?.toString() || item.price?.toString() || '',
        offerPrice: item.offerPrice?.toString() || '',
        gst: item.gst?.toString() || '5',
        packagingCharge: item.packagingCharge?.toString() || '',
        prepTime: item.prepTime || '20 mins',
        quantity: item.quantity || 50,
        availableToday: item.availableToday !== undefined ? item.availableToday : item.isAvailable !== undefined ? item.isAvailable : true,
        ...
        tags: item.tags || [],
        timeSlots: item.timeSlots || DEFAULT_TIME_SLOTS,
        dailyLimit: item.dailyLimit?.toString() || '100',
        lowStockAlert: item.lowStockAlert?.toString() || '10',
        autoDisable: item.autoDisable !== undefined ? item.autoDisable : true,
      });
    }
  }, [item]);
  ...
  const canSave = !!form.name && !!form.basePrice;
    ...
    <Modal visible={visible} animationType="slide" transparent statusBarTranslucent>
      <View style={s.overlay}>
        ...
        >
          ...
          <View style={s.header}>
            <View style={s.headerLeft}>
              ...
              <View>
                ...
                <Text style={s.headerSub}>Update details for employees & customers</Text>
              </View>
            </View>
            <TouchableOpacity
              style={s.closeBtn}
          ...
          {/* ── Sticky Footer ── */}
          <View style={s.footer}>
            <TouchableOpacity style={[s.draftBtn, { width: 100, justifyContent: 'center' }]} onPress={onClose} activeOpacity={0.8}>
              <Text style={s.draftText}>Cancel</Text>
            </TouchableOpacity>

            <TouchableOpacity
              style={[s.publishBtn, !canSave && s.publishBtnDisabled, { width: 100, justifyContent: 'center' }]}
              onPress={handleSave}
              activeOpacity={0.85}
              disabled={!canSave}
            >
              <MaterialIcons name="check" size={16} color="#fff" />
              <Text style={s.publishText}>Save</Text>
            </TouchableOpacity>
          </View>
